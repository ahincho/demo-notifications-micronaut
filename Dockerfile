# ============================================
# Nova Platform — demo-notifications-micronaut
# Multi-stage Dockerfile for the Micronaut demo.
#
# Build (from this directory):
#   docker buildx build \
#     --build-context hostm2=$env:USERPROFILE\.m2\repository \
#     -t demo-notifications-micronaut:1.0.0-SNAPSHOT .
#
# The `hostm2` build context injects the locally-published Nova libraries
# (core + micronaut-module) into the build container's Maven local repo.
# Prerequisite (one-time, and after any rebuild of the Nova core or module):
#   cd ..\..\java\nova-java-notifications
#   .\mvnw.cmd install
#   cd ..\..\java\nova-java-notifications-micronaut-module
...
ARG SERVICE_CODE=demo-notifications-micronaut
LABEL pe.edu.nova.git-sha="${GIT_SHA}" \
      ...
      org.opencontainers.image.source="https://github.com/ahincho/demo-notifications-micronaut" \
      org.opencontainers.image.title="demo-notifications-micronaut" \
      org.opencontainers.image.description="Nova Platform Micronaut demo: REST endpoint backed by nova-notifications-micronaut-module." \
...
# Copy the shadow jar.
COPY --from=builder --chown=nova:nova /build/build/libs/demo-notifications-micronaut-1.0.0-SNAPSHOT-all.jar /app/app.jar

USER nova
EXPOSE 8080
STOPSIGNAL SIGTERM

# TCP-only healthcheck (the demo has no /health endpoint).
